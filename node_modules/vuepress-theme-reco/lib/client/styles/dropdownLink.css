.dropdown-link {
  ...
  .dropdown-link__container {
    @apply pl-3 py-1 my-0 rounded-lg bg-basic list-none block;
    @apply md:px-3 md:py-4 md:my-3 md:border-block;
    /* @apply md:px-3 md:absolute md:top-12 md:-right-4 md:py-4 md:my-3 md:border-block; */
    .dropdown-link__item {
      @apply relative pb-1;
      ...
      &:not(:first-of-type) {
        .dropdown-link__subtitle {
          @apply relative z-30 mt-4 pt-1;
          &::before {
            @apply absolute left-0 -top-1 block w-full border-t border-solid border-reco-border-lightmode;
            @apply dark:border-reco-border-darkmode;
            content: '';
          }
        }
